...
class GoogleAuthService: NSObject {
    ...
    /// Performs the full OAuth flow: authorization, token exchange, and user info fetch.
    func authenticate() async throws -> GmailAccount {
        ...
        var account = GmailAccount(email: userInfo.email, displayName: userInfo.name)
    ...
    /// Refreshes the access token for a given account using its refresh token.
    func refreshAccessToken(for account: GmailAccount) async throws -> GmailAccount {
    ...
    /// Checks whether a real client secret has been configured.
    private func isClientSecretConfigured() -> Bool {
        return !Config.googleClientSecret.isEmpty
            && !Config.googleClientSecret.contains("YOUR_CLIENT_SECRET")
    }

    /// Encodes a dictionary as application/x-www-form-urlencoded data using RFC 3986.
